...
// IMPORTANT:
// This pattern ensures that in production builds, Rollup can fully drop the dev-only code path,
// meaning it will NOT try to build dev-only dynamic imports (../tests/*, ../../tests/*).
export const TestControlPanel = import.meta.env.DEV
  ? function TestControlPanelDev() {
      ...
      const runCommand = async (commandName: string, commandFn: () => Promise<any>) => {
        ...
        try {
          ...
          setLastResult(`❌ ${commandName} failed: ${error?.message ?? String(error)}`);
        } finally {
          setIsRunning(false);
        }
      };
      ...
      const commands = [
        {
          ...
          icon: Zap,
          description:
            '🚀 RUN THIS! Complete production readiness validation - runs all test suites (5-7 min)',
          category: 'main',
          action: async () => {
            const { runMasterTestSuite } = await import('../tests/master-test-suite');
            return await runMasterTestSuite(false);
          },
        },
        {
          ...
          icon: Zap,
          description:
            'Faster version - runs 24 core test suites, skips System Audit & Device Simulation (~3 min)',
          category: 'main',
          action: async () => {
            const { runMasterTestSuite } = await import('../tests/master-test-suite');
            return await runMasterTestSuite(true);
          },
        },
        {
          ...
          icon: Search,
          description:
            'Find existing test families (Family A, Family B) - run this first if no test data',
          category: 'setup',
          action: async () => {
        ...
        },
        {
          ...
          icon: Search,
          description:
            'Delete old test data and create fresh Family A + Family B (use if tests fail)',
          category: 'setup',
          action: async () => {
      ...
      return (
        <div className="fixed bottom-6 right-6 bg-white rounded-lg shadow-2xl z-50 w-[600px] max-h-[80vh] overflow-auto border border-gray-200">
          ...
          <div className="p-4 bg-blue-50 border-b border-blue-200">
            <p className="text-sm text-blue-900 font-medium mb-2">⚡ Quick Start (First Time):</p>
            <ol className="text-sm text-blue-800 space-y-1 ml-4">
              <li>
                1. Click <strong>"⚡ Quick Setup (Family A Only)"</strong> (creates test data in ~30 sec)
              </li>
              <li>
                2. Click <strong>"🎯 MASTER TEST SUITE"</strong> (runs all 27 tests)
              </li>
              <li>3. Check console for detailed results</li>
            </ol>
            <p className="text-xs text-blue-700 mt-2 italic">Already have test data? Skip to step 2!</p>
          ...
          <div className="sticky bottom-0 bg-gray-50 p-3 rounded-b-lg border-t border-gray-200">
            <p className="text-xs text-gray-600 text-center">💡 Tip: All results appear in the browser console</p>
          </div>
        </div>
      );
    }
  : function TestControlPanelProd() {
      return null;
    };
